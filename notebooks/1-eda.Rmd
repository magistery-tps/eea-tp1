...
```{r}
raw_train_set_count <- nrow(train_set)
train_set <- drop_missings(train_set)
missings_summary(train_set)
```

```{r}
print(paste('Total faltantes: ', (1 - (nrow(na.omit(train_set)) / raw_train_set_count)) * 100, "%"))
```
...
hist_plots(train_set, bins = c(7.5, 20, 20, 8.5, 7.5, 8.5))
```
