...
$E(peso) = \beta_0 + \beta_1 * altura + \beta_2 * edad + \beta_3 * genero + \beta4 * dias_actividad_fisica_semanal +  + \beta4 * consumo_diario_alcohol$
...
Leer el archivo "encuesta_salud_modelo6.csv". Este último consiste en el dataset original de train con la incorporación de algunas observaciones adicionales que pueden incluir valores atípicos. 
...
En el dataset de test original la variable peso tiene practicamente el doble de outliers que el dataset procesasdo.
...
Dada la presencia de outliers en la variable peso, el R^2 ajustado baja con respecto al modelo inicial.
...
Pregunta: A que se refiere con entrenar un modelo robusto con las misma especificaciones del modelo inicial????
